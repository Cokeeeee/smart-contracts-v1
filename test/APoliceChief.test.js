...
contract('PoliceChief', ([aliceAddress, bobAddress, carolAddress, devAddress, minterAddress]) => {
    beforeEach(async () => {
        this.cokeToken = await CokeToken.new({ from: aliceAddress });
        this.defaultBurnDivisor = 100 // 1% burn, changing this will break the tests
        await this.cokeToken.setBurnDivisor(this.defaultBurnDivisor, {from: aliceAddress})
    });

    it('should set correct state variables', async () => {
        this.policeChief = await PoliceChief.new(this.cokeToken.address, devAddress, '1000', '0', '1000', { from: aliceAddress });
        await this.cokeToken.transferOwnership(this.policeChief.address, { from: aliceAddress });
        const sushi = await this.policeChief.sushi();
        const devaddr = await this.policeChief.devaddr();
        const owner = await this.cokeToken.owner();
        assert.equal(sushi.toString(), this.cokeToken.address);
    ...
    it('should allow dev and only dev to update dev', async () => {
        ...
        await expectRevert(this.cokeToken.mint('0x0000000000000000000000000000000000000000', '100', { from: aliceAddress }), `ERC20: mint to the zero address.`);
        await this.cokeToken.mint('0x0000000000000000000000000000000000000001', '100')
    })
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.policeChief = await PoliceChief.new(this.cokeToken.address, devAddress, '100', '100', '1000', { from: aliceAddress });
        ...
        it('should give out SUSHIs only after farming time', async () => {
            ...
            assert.equal((await this.cokeToken.balanceOf(bobAddress)).toString(), '0');
            ...
            assert.equal((await this.cokeToken.balanceOf(bobAddress)).toString(), '990');
            await time.advanceBlockTo('104');
            await this.policeChief.deposit(0, '0', { from: bobAddress }); // block 105
            assert.equal((await this.cokeToken.balanceOf(bobAddress)).toString(), '4950');
            assert.equal((await this.cokeToken.balanceOf(devAddress)).toString(), '33');
            assert.equal((await this.cokeToken.totalSupply()).toString(), '4983');
            assert.equal((await this.cokeToken.totalSupplyBurned()).toString(), '50');
        });

        it('should not distribute SUSHIs if no one deposit', async () => {
            // 100 per block farming rate starting at block 200 with bonus until block 1000
            this.policeChief = await PoliceChief.new(this.cokeToken.address, devAddress, '100', '200', '1000', { from: aliceAddress });
            await this.cokeToken.transferOwnership(this.policeChief.address, { from: aliceAddress });
            await this.policeChief.add('100', this.lp.address, true);
            await this.lp.approve(this.policeChief.address, '1000', { from: bobAddress });
            await time.advanceBlockTo('199');
            assert.equal((await this.cokeToken.totalSupply()).toString(), '0');
            ...
            assert.equal((await this.cokeToken.totalSupply()).toString(), '0');
            assert.equal((await this.cokeToken.balanceOf(bobAddress)).toString(), '0');
            assert.equal((await this.cokeToken.balanceOf(devAddress)).toString(), '0');
            ...
            assert.equal((await this.cokeToken.totalSupply()).toString(), '9969');
            assert.equal((await this.cokeToken.balanceOf(bobAddress)).toString(), '9900');
            assert.equal((await this.cokeToken.balanceOf(devAddress)).toString(), '69');
            assert.equal((await this.lp.balanceOf(bobAddress)).toString(), '1000');
        });

        it('should distribute SUSHIs properly for each staker', async () => {
            // 100 per block farming rate starting at block 300 with bonus until block 1000
            this.policeChief = await PoliceChief.new(this.cokeToken.address, devAddress, '100', '300', '1000', { from: aliceAddress });
            await this.cokeToken.transferOwnership(this.policeChief.address, { from: aliceAddress });
            await this.policeChief.add('100', this.lp.address, true);
            ...
            await this.policeChief.deposit(0, '10', { from: aliceAddress });
            assert.equal((await this.cokeToken.totalSupply()).toString(), '10011');
            assert.equal((await this.cokeToken.balanceOf(aliceAddress)).toString(), '5610');
            assert.equal((await this.cokeToken.balanceOf(bobAddress)).toString(), '0');
            assert.equal((await this.cokeToken.balanceOf(carolAddress)).toString(), '0');
            assert.equal((await this.cokeToken.balanceOf(this.policeChief.address)).toString(), '4334');
            assert.equal((await this.cokeToken.balanceOf(devAddress)).toString(), '67');
            ...
            assert.equal((await this.cokeToken.totalSupply()).toString(), '20019');
            assert.equal((await this.cokeToken.balanceOf(aliceAddress)).toString(), '5610');
            assert.equal((await this.cokeToken.balanceOf(bobAddress)).toString(), '6129');
            assert.equal((await this.cokeToken.balanceOf(carolAddress)).toString(), '0');
            assert.equal((await this.cokeToken.balanceOf(this.policeChief.address)).toString(), '8144');
            assert.equal((await this.cokeToken.balanceOf(devAddress)).toString(), '136');
            ...
            assert.equal((await this.cokeToken.totalSupply()).toString(), '49846');
            assert.equal((await this.cokeToken.balanceOf(devAddress)).toString(), '343');
            // aliceAddress should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
            assert.equal((await this.cokeToken.balanceOf(aliceAddress)).toString(), '11485');
            // bobAddress should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
            assert.equal((await this.cokeToken.balanceOf(bobAddress)).toString(), '11714');
            // carolAddress should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
            assert.equal((await this.cokeToken.balanceOf(carolAddress)).toString(), '26303');
        ...
        it('should give proper SUSHIs allocation to each pool', async () => {
            ...
            this.policeChief = await PoliceChief.new(this.cokeToken.address, devAddress, '100', '400', '1000', { from: aliceAddress });
            await this.cokeToken.transferOwnership(this.policeChief.address, { from: aliceAddress });
            await this.lp.approve(this.policeChief.address, '1000', { from: aliceAddress });
        ...
        it('should stop giving bonus SUSHIs after the bonus period ends', async () => {
            ...
            this.policeChief = await PoliceChief.new(this.cokeToken.address, devAddress, '100', '500', '600', { from: aliceAddress });
            await this.cokeToken.transferOwnership(this.policeChief.address, { from: aliceAddress });
            await this.lp.approve(this.policeChief.address, '1000', { from: aliceAddress });
            ...
            assert.equal((await this.cokeToken.balanceOf(aliceAddress)).toString(), '10494');
        });
    });
});
