...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        await this.chef.deposit(0, '100', { from: minter });
        assert.equal((await this.sushi.totalSupply()).toString(), '99');
        assert.equal((await this.sushi.balanceOf(minter)).toString(), '99');
        assert.equal((await this.sushi.balanceOf(dev)).toString(), '0');
        ...
        assert.equal((await this.chef.poolLength()).toString(), '2');
    });
});
